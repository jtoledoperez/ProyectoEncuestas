...
public class EncuestaDAO {
	...
	public void save(Encuesta encuesta) {
		try (Session session = HibernateManager.getSessionFactory()
			...
			try {
				...
				System.err.println(
						"Error guardando la encuesta: " + e.getMessage());
				e.printStackTrace();
			}

		} catch (Exception e) {
			System.err.println(
					"Error abriendo la sesión de Hibernate: " + e.getMessage());
			e.printStackTrace();
		}
	}
	...
	public Encuesta getById(int id) {
		try (Session session = HibernateManager.getSessionFactory()
			...
			System.err.println(
					"Error obteniendo la encuesta por ID: " + e.getMessage());
			e.printStackTrace();
			return null;
		}
	}
	...
	public List<Encuesta> getAll() {
		try (Session session = HibernateManager.getSessionFactory()
			...
			Query<Encuesta> query = session.createQuery(
					"FROM Encuesta e JOIN FETCH e.usuario", Encuesta.class);
			return query.list();
		} catch (Exception e) {
			System.err.println(
					"Error obteniendo las encuestas: " + e.getMessage());
			e.printStackTrace();
			return null;
		}
	}
	...
	public boolean update(Encuesta encuesta) {
		try (Session session = HibernateManager.getSessionFactory()
			...
			try {
				...
				transaction.rollback();
				System.err.println(
						"Error actualizando la encuesta: " + e.getMessage());
				e.printStackTrace();
				return false;
			}
		}
	...
	public void delete(Encuesta encuesta) {
		try (Session session = HibernateManager.getSessionFactory()
			...
			try {
				...
				transaction.rollback();
				System.err.println(
						"Error eliminando la encuesta: " + e.getMessage());
				e.printStackTrace();
			}
		}
	}
