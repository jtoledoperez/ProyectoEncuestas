<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"
	import="java.util.List, java.util.Map, modelo.Encuesta"%>
<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
...
<body>
	...
	<div class="container mt-4">
		...
		Map<modelo.Pregunta, List<modelo.Respuesta>> preguntaRespuestasMap = (Map<modelo.Pregunta, List<modelo.Respuesta>>) request
				.getAttribute("preguntaRespuestasMap");

		if (encuesta != null) {
		%>
		<h2 class="mb-3">
			Encuesta:
			<%=encuesta.getNombre()%></h2>
		<div class="mb-3">
			<strong>Respuestas contestadas: <span
				id="contadorRespuestas">0</span> de <%=preguntaRespuestasMap.size()%></strong>
			<div class="progress">
				<div class="progress-bar" role="progressbar" id="barraProgreso"
					style="width: 0%;" aria-valuenow="0" aria-valuemin="0"
					aria-valuemax="<%=preguntaRespuestasMap.size()%>"></div>
			</div>
		</div>
		...
		<form onsubmit="return redirigirAIndex()" method="post">
				...
				<div class="card-body">
					...
					<div class="form-check">
						<input class="form-check-input" type="radio"
							...
							onchange="actualizarContador()"> <label
							class="form-check-label"> <%=respuesta.getTexto()%>
						</label>
					</div>
					<%
					}
					...
					%>
					<p class="text-muted">No hay respuestas disponibles para esta
						pregunta.</p>
					<%
					}
					%>
			...
			%>
			<div class="alert alert-warning" role="alert">No hay preguntas
				disponibles para esta encuesta.</div>
			<%
			}
			%>
		...
		%>
		<div class="alert alert-danger" role="alert">No se pudo cargar
			la encuesta.</div>
		<%
		}
		%>
